...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
      ...
      for (int t = 0; t < N - 2; t++) {
        fg[0] += 150.0*CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
        fg[0] += 15.0*CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
      }
...
vector<double> MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs) {
  ...
  std::vector<double> control_and_prediction;
  control_and_prediction.push_back(solution.x[delta_start]);
  control_and_prediction.push_back(solution.x[a_start]);

  for(int i = 0; i < N; i++)
  {
      control_and_prediction.push_back(solution.x[x_start + i]);
      control_and_prediction.push_back(solution.x[y_start + i]);
  }

  return control_and_prediction;
}
