    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          Eigen::Transform<double,2,Eigen::Affine> tfm;
          tfm.setIdentity();
          tfm.translate(Eigen::Vector2d(px, py));
          tfm.rotate(Eigen::Rotation2D<double>(psi));

          std::vector<Eigen::Vector2d> points_reference;
          Eigen::VectorXd xvals(ptsx.size());
          Eigen::VectorXd yvals(ptsy.size());

          for(int i = 0; i < ptsx.size(); i++)
          {
              Eigen::Vector2d pt(ptsx[i], ptsy[i]);
              pt = tfm.inverse() * pt;
              points_reference.push_back(pt);
...
//          double cte = -polyeval(coeffs, xvals[0]);
//          double epsi = 0 - atan(2*coeffs[2]+coeffs[1]);
          double cte = coeffs[0];
          ...
          */
          ...
          for(int i = 0; i < (vars.size()-2)/2; i++)
          {
            ...
            //mpc_x_vals.push_back((double)i/2);
            //mpc_y_vals.push_back((double)i/2);
          }
